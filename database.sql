-- ============================================================================
-- ContractIQ — Production Database Schema (consolidated)
--
-- Single paste-and-run file for the Supabase SQL Editor. Idempotent where
-- practical (IF NOT EXISTS / ON CONFLICT / guarded DO blocks / DROP+CREATE
-- for triggers and policies) so it is safe to re-run on the same project.
--
-- This file is the consolidation of:
--   - docs/specs/supabase-schema.sql               (core schema)
--   - docs/specs/supabase-schema-additions.sql      (correction_review_queue)
-- Those two files remain the source of truth for each part; this file exists
-- so the whole database can be stood up from one script. If either source
-- file changes, regenerate this file to match rather than editing it by hand
-- and letting it drift.
--
-- Source: docs/engineering/engineering-doc.md, Section 7 (Database Design)
-- and Section 8 (AI Architecture — rate limiting / cost / realtime); and
-- docs/specs/13-rate-limiting-and-cost-control.md (correction_review_queue).
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- 13. TABLE: correction_review_queue
--
-- Weekly drift-review sample written by netlify/functions/quality-monitor.ts
-- (docs/specs/13-rate-limiting-and-cost-control.md, writeToReviewQueue()).
-- Populated exclusively from the already-anonymised term_corrections view
-- (no user_id column here either); read by the ops team, never by an end
-- user's browser.
-- ----------------------------------------------------------------------------
...
-- (contract | history | both) produced an assistant reply, so the UI can
-- attribute the source correctly on reload and Realtime delivery, not just
-- in the initial HTTP response. Null for user-authored rows.
...
-- ----------------------------------------------------------------------------
-- 15. STORAGE — `contracts` bucket + RLS policies
--
...
-- policies (created above) now that lib/security/rateLimiter.ts reads and
-- writes this table exclusively via the service-role client. Without these
-- policies, a user hitting the Supabase REST API directly with their own JWT
-- — bypassing this app's routes entirely — cannot read or write this table
-- at all.
drop policy if exists "rate_limit_events_select_own" on rate_limit_events;
drop policy if exists "rate_limit_events_insert_own" on rate_limit_events;
